...
public class NPCBulletScript : MonoBehaviour
{
    ...
    public void InitializeProjectile(float trajectoryMaxHeight, float distanceToTargetToDestroyProjectile, float maxMoveSpeed, float destroyTime, Transform target)
    {
        //mainCam = GameObject.FindGameObjectWithTag("MainCamera").GetComponent<Camera>();
        //mousePos = mainCam.ScreenToWorldPoint(Input.mousePosition);
        this.target = target;
